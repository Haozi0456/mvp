package com.zwh.mvp.library.tools.glideModule;
...
public class ImageCatchUtil {
    ...
    public static ImageCatchUtil getInstance(Context context) {
        if (inst == null) {
            inst = new ImageCatchUtil(context);
        }
        return inst;
    }

    private ImageCatchUtil(Context context) {
        this.context = context;
        ImageExternalCatchDir = context.getExternalCacheDir().getPath();
    }
    ...
    public long getCacheSize() {
        try {
            return getFolderSize(new File(ImageExternalCatchDir));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return 0;
